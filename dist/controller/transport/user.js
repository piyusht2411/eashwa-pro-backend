...
const createTransportUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!allowed.includes(role)) {
            return res.status(400).json({ message: "Role must be admin | accounts | driver" });
        }
        ...
        const user = yield user_1.default.create({ name, email, password, role, phone: phone || "", portal: "transport" });
        return res.status(201).json({
            message: "User created successfully",
            user: { _id: user._id, name: user.name, email: user.email, role: user.role, portal: user.portal, phone: user.phone },
        });
    }
    catch (err) {
...
const getTransportUserById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const user = yield user_1.default.findOne({ _id: req.params.id, portal: "transport" }).select("-password -fcmToken");
        if (!user)
...
const updateTransportUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (email !== undefined) {
            const exists = yield user_1.default.findOne({ email: email.toLowerCase().trim(), _id: { $ne: id } });
            if (exists)
        ...
        return res.status(200).json({
            ...
            user: { _id: user._id, name: user.name, email: user.email, role: user.role, portal: user.portal, phone: user.phone, isActive: user.isActive },
        });
    }
    catch (err) {
        return res.status(500).json({ message: err.message });
